...
# Export
write.csv(sub_conflict_matrix, "4/sub_conflict_matrix", row.names = TRUE)
...
E(g)$weight <- ifelse(E(g)$type == "cooperation", 5, 1)
...
layout <- create_layout(as_tbl_graph(g), layout = "fr", weights = NULL)
...
ggraph(layout) +
  ...
  scale_edge_color_manual(values = c(cooperation = "steelblue", conflict = "firebrick")) +
  geom_node_point(aes(fill = cluster), shape = 21, size = 5, stroke = 0.5, color = "black") +
  geom_node_text(aes(label = name), repel = TRUE, size = 5) +
